...
LPCSTR CPersist::WND_PLACE	= "WP";

void CPersist::SaveWnd(LPCTSTR Section, const CWnd *Wnd, LPCSTR Name)
{
	WINDOWPLACEMENT	wp;
...
int CPersist::LoadWnd(LPCTSTR Section, CWnd *Wnd, LPCSTR Name, int Options)
{
	WINDOWPLACEMENT	wp;
...
int CPersist::GetWndShow(LPCTSTR Section, LPCSTR Name)
{
	WINDOWPLACEMENT	wp;
	DWORD	Size = sizeof(WINDOWPLACEMENT);
	memset(&wp, 0, Size);
...
{
	CString	KeyName;
	KeyName.Format("Software\\%s\\%s\\%s", AfxGetApp()->m_pszRegistryKey, 
